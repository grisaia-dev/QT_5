...
void MainWindow::StartRace(void) {
    if(ui->rb_qtConcur->isChecked()){
        ...
        fnumber = QtConcurrent::run(&ExampleRace::DoWork, concurRace1, &number, ui->rb_mutexOn->isChecked(), ui->sb_initNum->value());
        while (fnumber.isFinished() == false) {continue;}
        fnumber = QtConcurrent::run(&ExampleRace::DoWork, concurRace2, &number, ui->rb_mutexOn->isChecked(), ui->sb_initNum->value());
        while (fnumber.isFinished() == false) {continue;}
